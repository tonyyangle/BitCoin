...
public class OKCoinApiClient {
	...
	private final static long STATUS = 1;
	...
	private final static String TRADE_URI = "https://www.okcoin.cn/api/trade.do";
	private final static String TYPE = "buy";
	private final static String AMOUNT = "0.01";

	public static String testApi(String inputAmount, String inputPrice) {

		// 参数数组
		Map<String, String> sArray = new HashMap<String, String>();
		sArray.put("partner", Long.toString(PARTNER));
		sArray.put("symbol", SYMBOL);
		sArray.put("type", TYPE);
		sArray.put("rate", inputPrice);
		sArray.put("amount", inputAmount);
		...
		try {
			...
			// post请求
			PostMethod method = new PostMethod(TRADE_URI);
			NameValuePair[] data = { new NameValuePair("amount", inputAmount),
					new NameValuePair("partner", Long.toString(PARTNER)),
					new NameValuePair("rate", inputPrice),
					new NameValuePair("symbol", SYMBOL),
					new NameValuePair("type", TYPE),

					// new NameValuePair("status", Long.toString(STATUS)),
					// new NameValuePair("currentPage",
					// Long.toString(CURRENTPAGE)),
					// new NameValuePair("pageLength",
					// Long.toString(PAGELENGTH)),
